...
class AddGaussianNoise:
    ...
    def __call__(self, img, mask):
        img = np.array(img)
        std = random.uniform(.005, self.std_max)  # random uniform noise std
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(mean={0}, std={1})'.format(self.mean, self.std_max)
...
class Resize(torch.nn.Module):
    def __init__(self, size, img_interpolation=torchvision.transforms.InterpolationMode.BILINEAR,
    ...
    def forward(self, img, mask):
        ...
        resized_img = F.resize(img, self.size, self.img_interpolation)
        resized_mask = F.resize(mask, self.size, self.mask_interpolation)

        # # Convert PIL Image to PyTorch tensor if necessary
        if isinstance(resized_mask, Image.Image):
            to_tensor = transforms.ToTensor()
            resized_mask = to_tensor(resized_mask)

        # Now you can convert to NumPy safely
        # resized_mask_np=np.asarray(resized_mask).squeeze()
        resized_mask_np = resized_mask.cpu().numpy().squeeze()

        # Apply thresholding
        resized_mask_np[resized_mask_np >= 0.5] = 1
        resized_mask_np[resized_mask_np < 0.5] = 0

        kernel = np.ones((2, 2), np.uint8)
        resized_mask_np = cv2.erode(resized_mask_np, kernel, iterations=1)

        # kernel = np.ones((5, 5), np.uint8)
        # opened_mask = cv2.morphologyEx(resized_mask_np, cv2.MORPH_OPEN, kernel)

        # Remove small connected components
        cleaned_mask = self.remove_small_objects(resized_mask_np, min_size=15)

        # Convert cleaned_mask back to PIL Image
        cleaned_mask_img = Image.fromarray(255*np.uint8(cleaned_mask))

        return resized_img, cleaned_mask_img
...
class RandomAdjustSharpness(torch.nn.Module):
    ...
    def forward(self, img,mask):
        ...
        if torch.rand(1).item() < self.p:
            return F.adjust_sharpness(img, self.sharpness_factor),mask
        return img,mask
